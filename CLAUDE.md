...
- **Card tab:** the memdeck card, big (`screens/CardScreen.tsx`) — a fresh card every time the tab opens, tap for another, *Think of a card* deals one ten seconds later. `grabber://card` (the lock-screen widgets' tap) lands here. Deals through `WidgetBridge`, syncs the widgets once on leaving.
