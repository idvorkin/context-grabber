...
- **Widgets (`ios/LiveActivity/`):** Today (medium/large; the large one carries the memdeck card) and the lock-screen Memdeck card. The card is a function of the clock (one per five minutes) and one shared tap count in the App Group (a tap on the card deals a new one, `DealCardIntent.swift`), so every surface deals the same card with nothing else shared (`PlayingCard.swift`; its promises are checked under plain `swiftc`, not jest). Spec: `docs/superpowers/specs/2026-09-07-widget-random-card-design.md`.
